...
export class AppResizableComponent implements OnInit {
  ...
  public efnOnDrop(event: any): void {
    ...
    console.log(nActiveTransform);
    if (
      this.eLayout === ResizableLayoutEnum.Right ||
      this.eLayout === ResizableLayoutEnum.Bottom
    ) {
      this.nTransformedPosition =
        this.oSplitterPosition.nInitialPosition + nActiveTransform;
      console.log('Trasnformed ' + this.nTransformedPosition);
      if (nActiveTransform < this.nMinTransformAllowed) {
        this.fnSetSplitterPosition(
          event,
          this.nMinTransformAllowed,
          this.oSplitterPosition.nMinPosition
        );
      }
      if (nActiveTransform > this.nMaxTransformAllowed) {
        this.fnSetSplitterPosition(
          ...
          this.oSplitterPosition.nMaxPosition
        );
      }
    } else {
      this.nTransformedPosition =
  ...
  private fnSetMinAndMaxTranformationAllowed(): void {
    if (
      ...
      this.nMinTransformAllowed =
        this.oSplitterPosition.nMinPosition - this.nTransformedPosition;
      this.nMaxTransformAllowed =
        this.oSplitterPosition.nMaxPosition - this.nTransformedPosition;
    } else {
      this.nMaxTransformAllowed =
        this.nTransformedPosition - this.oSplitterPosition.nMinPosition;
      this.nMinTransformAllowed =
        this.nTransformedPosition - this.oSplitterPosition.nMaxPosition;
    }
  }
